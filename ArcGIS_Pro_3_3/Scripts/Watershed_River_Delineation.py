...
if usegui == False:
    indir = r"E:\UT_Reservoir_sedimentation\Input_Data"
    # Input DEM (raster)
    in_dem = os.path.join(indir, 'DEM', 'UT_DEM_utm.tif')
    ...
    in_pourpoint = os.path.join(indir, 'UT_watersheds_pourpoints', 'UT_watersheds_PPV2.shp')
    # name of field with pour point identifiers
    ppfieldin = "UID"
    # pour point id INPUT AS STRING; 
    pp_id = '0'  # '73,75'#if using multiple seperate wtih comma or semicolon ex '0,3,97'
    # tolerance to shift pour point
    pp_tol = 30

    # Input area of interest shapefile ( polygon shapefile)
    in_aoi = os.path.join(indir, 'UT_watersheds_pourpoints', 'UT_WTRSHDS_Complete_100mBuffer_utmV3.shp')
    # name of the field housing aoi of interest identifiers
    aoifield = "UID"
    # aoi id input as a string 
    aoi_id = '0'  # '34,25' #if using multiple seperate with a comma or semicolon

    # Set output directories and filename
    # Directory to house output data
    out_dir = r"E:\UT_Reservoir_sedimentation\eval\extfix\a000v2"
    # Base file name (name or id of basin you are working with)
    out_fid = 'a000'

    isnested = False
    nestfid = 'a073,a075'  # names oor outputs of nested basins
...
def findcellulararea_sqkm(in_ras):
    ...
    singcellras = arcpy.sa.SetNull(in_ras, in_ras, '"VALUE"<' + str(maxrasval))
    IntRas = arcpy.sa.Int(singcellras)
    poly = os.path.join(out_tmp_id + 'ptest.shps')  # r"in_memory\poly"
    arcpy.RasterToPolygon_conversion(IntRas, poly,
...
if isnested == True:
    ...
    for i in np.arange(len(pp_id)):
        ...
        outfilew = nestout + '_watershed.shp'
        outfilewdir, outfilewfid = os.path.split(outfilew)
        arcpy.conversion.FeatureClassToFeatureClass(lyridwtr, nestdir, outfilewfid)
        arcpy.management.AddField(outfilew, "Id")
        arcpy.management.CalculateGeometryAttributes(outfilew, "area_km2 AREA", '', 'SQUARE_KILOMETERS')
        flds = arcpy.ListFields(outfilew)
        keepflds = ['Id', 'FID', 'Shape', 'area_km2']
        for field in flds:
            # print(field.name)
            if field.name not in keepflds:
                try:
                    arcpy.DeleteField_management(outfilew, field.name)
                except:
                    print('The following field could not be deleted ', field.name)
        ...
        # arcpy.Clip_analysis(wtrshd, lyridwtr, outfilew)
        # arcpy.env.extent=outfilew
        ...
        for file in rasfiles:
            ...
            outExtractByMask = arcpy.sa.ExtractByMask(file, outfilew)
            # Save the output 
        ...
        for file in shpfiles:
            if file != wtrshd:
                ...
                outfile = nestout + fileid[len(out_fid):]
                # Execute Clip
                arcpy.Clip_analysis(file, outfilew, outfile)

if isnested == False:
    try:
        arcpy.DeleteField_management(wtrshd, 'gridcode')
    except:
        pass
    arcpy.management.CalculateGeometryAttributes(wtrshd, "area_km2 AREA", '', 'SQUARE_KILOMETERS')
